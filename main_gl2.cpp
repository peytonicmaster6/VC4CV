...
int main(int argc, char **argv)
{
	...
	//params.camera_num = 1;
	//std::cout << "Camera Number " << params.camera_num << "\n";
	camGL1 = camGL_create1(eglSetup, (const CamGL_Params*)&params);
	//printf("we made it\n");
	if (camGL == NULL)
	{
	...
	{ // Start CamGL
		...
		int status1 = camGL_startCamera1(camGL1);
		...
		{ // Process incoming frames
			...
			{
				while((status = camGL_nextFrame(camGL1)) == CAMGL_SUCCESS)
				{ // Frames was available and has been processed
					...
					//go to 480, 0 for a preview of 960, 1080.
					...
					//glBindFramebuffer(GL_FRAMEBUFFER, 0);
					SSQuad->draw();
					//eglSwapBuffers(eglSetup.display, eglSetup.surface); (dont need causes flickering)
					...
					{ // Check for keys
						char cin;
						if (read(STDIN_FILENO, &cin, 1) == 1)CamGL *camGL1;
						{
			...
			if (status1 != 0)
				...
				camGL_stopCamera1(camGL1);
		}
		camGL_destroy(camGL);
		camGL_destroy1(camGL1);
		terminateEGL(&eglSetup);
