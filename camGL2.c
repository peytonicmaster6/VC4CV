...
CamGL *camGL_create1(EGL_Setup EGLSetup, const CamGL_Params *params)
{
	VCOS_STATUS_T vstatus;
	int istatus;
	...
	// Allocate CamGL structure
	CamGL *camGL1 = vcos_calloc(1, sizeof(*camGL1), "camGL");
	CHECK_STATUS_V((camGL1? VCOS_SUCCESS : VCOS_ENOMEM), "Memory allocation failure", error_ctx);
	camGL1->params = *params;
	camGL1->eglSetup = EGLSetup;

	// Initialize Semaphores
	vstatus = vcos_mutex_create(&camGL1->accessMutex, "camGL-access");
	CHECK_STATUS_V(vstatus, "Error creating semaphore", error_mutex1);

	// Init EGL
	istatus = camGL_initGL(camGL1);
	CHECK_STATUS_V(istatus, "Error initialising EGL", error_gl);
	...
	camGL1->gcs = gcs_create1(&gcsParams);

	CHECK_STATUS_V(camGL1->gcs? 0 : 1, "Error initialising GCS1", error_gcs);
...
error_gcs:
	camGL_stopGL(camGL1);
error_gl:
	vcos_mutex_delete(&camGL1->accessMutex);
error_mutex1:
	vcos_free(camGL1);
error_ctx:
	return NULL;
...
void camGL_destroy1(CamGL *camGL1)
{
	gcs_destroy1(camGL1->gcs);
	camGL_stopGL(camGL1);
	vcos_mutex_delete(&camGL1->accessMutex);
	vcos_free(camGL1);
}
...
/** Start camera */
uint8_t camGL_startCamera1(CamGL *camGL1)
{
	CHECK_GL(camGL1);
	if (camGL1->error)
	{
...
/** Explicitly stop gl camera stream */
uint8_t camGL_stopCamera1(CamGL *camGL1)
{
	uint8_t code = CAMGL_SUCCESS;
	...
	if (camGL1->started)
	{
		...
		// Stop
		camGL1->started = false;
		camGL_setQuit(camGL1, false);
		gcs_stop1(camGL1->gcs);
	...
	{
		glClear(GL_COLOR_BUFFER_BIT);
		eglSwapBuffers(camGL1->eglSetup.display, camGL1->eglSetup.surface);
	}
	glFlush();
	CHECK_GL(camGL1);
...
/* Returns whether there is a new camera frame available */
uint8_t camGL_hasNextFrame1(CamGL *camGL1)
{
	return gcs_hasFrameBuffer1(camGL1->gcs);
}
 ...
 * If no camera frame is available yet, blocks until there is.
 * If the last frame has not been returned yet or camera stream was interrupted, returns error code. */
int camGL_nextFrame1(CamGL *camGL1)
{
	if (!camGL1->started)
	{
		vcos_log_error("Camera not started before requesting frames!");
	...
	if (camGL_getQuit(camGL1))
	{
		uint8_t code = camGL_stopCamera1(camGL1);
		return code? CAMGL_QUIT : code;
	}

	gcs_returnFrameBuffer1(camGL1->gcs);

	void *cameraBufferHeader = gcs_requestFrameBuffer1(camGL1->gcs);
	if (cameraBufferHeader)
	{
		void *cameraBuffer = gcs_getFrameBufferData1(cameraBufferHeader);
		if (camGL_processCameraFrame(camGL1, cameraBuffer) == 0)
			return CAMGL_SUCCESS;
		vcos_log_error("Failed to process frame!");
		return CAMGL_ERROR;
	}
